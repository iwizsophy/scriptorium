...
func TestHelperPredicatesCoverRemainingBranches(t *testing.T) {
	rootPath := filepath.Join(string(filepath.Separator), "repo")
	childPath := filepath.Join(rootPath, "child.txt")
	otherPath := filepath.Join(string(filepath.Separator), "other", "child.txt")

	if !isWithinRoot(rootPath, childPath) {
		t.Fatal("expected child path to remain within root")
	}
	if isWithinRoot(rootPath, otherPath) {
