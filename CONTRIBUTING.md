...
- Maintainers may create or update a matching markdown tracker under `issues/`
  for substantial work because this repository still uses in-repo issue files as
  its implementation ledger.
...
2. Open or update an issue when the work changes behavior, architecture, or
   repository policy.
3. If the work is substantial, mirror it in `issues/*.md` or update the
   existing tracker if one already exists.
4. Implement the change with matching tests or documentation updates.
5. Run local validation from the repository root:
   - `go test ./...`
   - `go build ./cmd/...`
