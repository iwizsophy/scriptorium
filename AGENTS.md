...
6. Track non-trivial work items as markdown issue files under `issues/`. Large changes must have an issue file before or with the implementation change.
...
20. When new follow-up work, open questions, or carry-over tasks are discovered, create a new issue file under `issues/` and record the remaining work before closing the current issue.
...
4. Create or update the relevant file in `issues/`.
...
- Work items are tracked as markdown issue files under `issues/`.
- Public issue and pull request intake now exists through GitHub community files under `.github/`, while implementation tracking still uses markdown issue files under `issues/`.
