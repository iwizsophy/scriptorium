...
- Update behavior docs, operator docs, and issue tracking in the same work item when behavior or policy changes.
- Large or non-trivial work must have a matching open GitHub issue.
...
- When issue scope is complete, mark the issue `done` or closed before starting the next issue.
